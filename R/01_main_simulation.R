...
masterSimulation <- function() {
      ...
      logger::log_success("Simulation done!!")
      ...
      for (i in seq_along(sModelName)) {
            ...
            for (j in seq_along(vAllSims)) {
               ...
               logger::log_info("--- Simulation: {j}/{length(vAllSims)} ---")
               ...
               if (length(dir(sSimRaw)) == 0) {
                  ...
                  logger::log_info("Write parquet #2")
                  arrow::write_parquet(tblSummary,
                                       glue::glue("{sSimSummary}/{nSimRunSave}_{sModelSaveName}_summary.parquet"))

                  rm(list = c("stanObj", "stanSummary", "tblSummary"))
                  gc()

               } else {

                  logger::log_info("data already extracted")
