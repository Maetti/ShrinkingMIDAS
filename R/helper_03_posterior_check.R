...
md_check_posterior_mse_beta_plot <- function(tblInput) {
   ...
   sSimLabels <- rep("", max(tblInput$simulation))
   names(sSimLabels) <- sort(tblInput$simulation %>% unique())
   sSimLabels[c(1, seq(0, max(tblInput$simulation), 5)[-1])] <- c(1, seq(0, max(tblInput$simulation), 5)[-1])
