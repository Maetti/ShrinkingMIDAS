...
moc_yrep_stat_violin_facet <- function(tblStat, sStat, sTitle = "Mean",
   ...
   sSimLabels <- rep("", max(tblPlot$simulation))
   names(sSimLabels) <- sort(tblPlot$simulation %>% unique())
   sSimLabels[c(1, seq(0, max(tblPlot$simulation), 5)[-1])] <- c(1, seq(0, max(tblPlot$simulation), 5)[-1])
   ...
   ggplot2::ggplot(data = tblPlot, ggplot2::aes(x = factor(simulation), y = value, color = pInOut)) +
      ...
      ggplot2::labs(color = sLegend) +
      ggplot2::scale_color_manual(values = c("In" = "#67a9cf", "Out" = "#ef8a62")) +
      ggplot2::scale_x_discrete(labels = sSimLabels) +
...
moc_yrep_ecdf_per_y <- function(tblStat, sTitle = "F(X)",
   ...
   nSimulationMax <- max(as.numeric((tblStat$simulation)))
   sSimLabels <- rep("", nSimulationMax)
   names(sSimLabels) <- sort(tblStat$simulation %>% unique())
   sSimLabels[c(1, seq(0, nSimulationMax, 5)[-1])] <- c(1, seq(0, nSimulationMax, 5)[-1])
...
moc_yrep_intervall_cover <- function(tblInput, nIntervall = 0.9,
   ...
   nSimulationMax <- max(as.numeric((tblPlot$simulation)))
   sSimLabels <- rep("", nSimulationMax)
   names(sSimLabels) <- sort(tblPlot$simulation %>% unique())
   sSimLabels[c(1, seq(0, nSimulationMax, 5)[-1])] <- c(1, seq(0, nSimulationMax, 5)[-1])
