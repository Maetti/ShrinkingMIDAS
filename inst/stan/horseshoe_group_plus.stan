...
parameters {
  ...
  // additional prior for the "+" in the horseshoe
  real<lower=0> delta_prior[nG]; 
...
model {
  ...
  // group
  delta_prior ~ cauchy(0, 1);
  delta ~ cauchy(0, delta_prior);

  // local
  lambda_prior ~ cauchy(0, 1);
  lambda ~ cauchy(0, lambda_prior);
